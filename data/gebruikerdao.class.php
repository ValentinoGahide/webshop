...
class GebruikerDAO {
    ...
    public static function getAll() {
        ...
        $sql = "select gebruikers_pz.id as gebr_id, naam, wachtwoord, usernaam, 
        ...
        foreach ($resultSet as $rij) {
            ...
            $gebruiker = Gebruiker::create($rij["gebr_id"], $rij["naam"], $rij["usernaam"], $rij["wachtwoord"],
    ...
    public static function getByUsernm($usernaam) {
        ...
        $sql = "select gebruikers_pz.id as gebr_id, naam, wachtwoord, usernaam, 
                  ...
                  from gebruikers_pz,  postcodes 
                 where postcode_id =  postcodes.id
                   and gebruikers_pz.usernaam = " . "'" . $usernaam . "'";
        $resultSet = $core->dbh->query($sql);
        $rij = $resultSet->fetch();
        if ($rij) {
           $postcode  = Postcode::create($rij["postcode_id"], $rij["postnr"], $rij["gemeente"], $rij["kostprijs"], $rij["thuis_lev_ok"]);
    ...
    public static function Bestaat_Nieuwe_Gebr_al($usernaam) {
        $core = DBConfig::getInstance();
        $sql = "select gebruikers_pz.id as gebr_id, naam, wachtwoord, 
                       level_auth, adres, email, telefoon, actief, 
                       dt_aangemaakt, postcode_id, korting, opm_extra, btwnr,
                       postcodes.gemeente as gemeente, postnr, kostprijs, thuis_lev_ok
                  from gebruikers_pz,  postcodes 
    ...
    public static function Bestaat_Gebr_Pasw($usernaam, $wachtwoord) {
        ...
        $sql = "select gebruikers_pz.id as gebr_id, naam, wachtwoord, 
                 ...
                 where postcode_id =  postcodes.id
                   and gebruikers_pz.usernaam = " . "'" . $usernaam . "'" . 
                   " and gebruikers_pz.wachtwoord = " . "'" . $wachtwoord . "'" ;
        $resultSet = $core->dbh->query($sql);
        $rij = $resultSet->fetch();
        if ($rij) {
    ...
    public static function Get_Gebr_Actief($usernaam) {
        ...
        $sql = "select actief
                  from gebruikers_pz
                 where gebruikers_pz.usernaam = " . "'" . $usernaam . "'" ;
        $resultSet = $core->dbh->query($sql);
        $rij = $resultSet->fetch();
        if ($rij) {
           $dbh = null;
    ...
    public static function Get_Gebr_Level_User ($usernaam) {
        ...
        $sql = "select level_auth
                  from gebruikers_pz
                 where gebruikers_pz.usernaam = " . "'" . $usernaam . "' limit 1"; 
        $resultSet = $core->dbh->query($sql);
        $rij = $resultSet->fetch();
        if ($rij) {
           $dbh = null;
    ...
    public static function Create($naam, $usernaam, $wachtwoord, $level_auth, $adres, 
        ...
        $core = DBConfig::getInstance();
        $sql = "insert into gebruikers_pz (naam, usernaam, wachtwoord, level_auth, adres, email, telefoon, 
        ...
        $gebruiker = Gebruiker::create($Gebr_Id, $naam, $usernaam, $wachtwoord, $level_auth, $adres, 
                                  $email, $telefoon, $actief, $dt_aangemaakt, 
